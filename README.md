...
python scripts/generate_controls.py \
  ...
  --seed 42 \
  --seed-count 10 \
  --include-bisected-lozenge-controls \
  --bisect-fraction 0.5 \
  --bisect-mode mixed \
  --svg-previews
```

This writes clean lozenge/rhombic lattice, broken/noisy lozenge lattice, random scratch, and curved random scratch PNG controls plus `expected_metadata.json`. With `--include-bisected-lozenge-controls`, it also writes `bisected_lozenge_lattice.png`, starting from the lozenge lattice and adding split strokes across a configurable fraction of cells. The metadata labels this as `class="bisected_lozenge"` and `motif="split_lozenge"` so it is aggregated separately from plain lozenge and random controls. With `--seed-count > 1`, each seed is written to its own `seed_####/` directory and a `control_seed_manifest.json` records the generated set. The default controls are bright strokes on a dark background, matching the threshold pipeline polarity; use `--control-polarity dark-on-bright` only for downstream tools that explicitly expect that polarity. Clean synthetic lozenge lattices and split/bisected lozenge lattices are separate positive controls because they test different motifs. SandDraw/iPad sand-texture images are not positive controls unless a preprocessing step first isolates actual groove centerlines; otherwise they test rendering texture, not extracted engraving topology.
...
The comparison writes `comparison_summary.csv`, `comparison_summary.json`, `invalid_controls.csv`, distribution PNG plots for normalized metrics, `lozenge_candidates.csv`, `open_lozenge_candidates.csv`, `bisected_lozenge_candidates.csv`, and `lozenge_summary.json`. Metrics include normalized endpoint and junction rates per 1000 traced pixels, cycles per node, triangle counts and triangle regularity, bisected-lozenge candidates from adjacent triangle pairs, 4-cycles per node, lozenge candidates per cycle, degree-3 and degree-4 fractions, orientation entropy, orientation peak concentration, largest connected component fraction, and supporting raw counts. Control results are aggregated by class in `class_aggregates`, with separate entries for `random`, `lozenge`, and `bisected_lozenge` when those controls are present. Each class includes sample counts, means, standard deviations, ranges, and values for reproducibility. When a control directory contains `metrics.json`, `graph_raw.graphml`, and `graph_merged.graphml`, the comparison uses the merged graph as one control sample and ignores raw graphs by default to avoid double-counting. Controls with `node_count == 0` or `total_traced_length == 0` are reported separately as invalid and excluded from class means.
...
Triangle metrics are reported separately from four-cycle and lozenge metrics. `triangle_count`, `triangles_per_node`, `triangles_per_cycle`, triangle side-length coefficient of variation, and triangle area summaries describe triangular bracing or triangular mesh motifs. Some triangles may also be substructure inside a rhomboid cell: `bisected_lozenge_candidate_count`, candidate ratios, and mean confidence look for adjacent triangle pairs that share one edge and whose outer boundary is a plausible split lozenge/rhomboid. Interpret those as bisected-cell evidence, not as independent triangular motifs competing with lozenge structure.

...
def generate_control_set(
    ...
    clean_meta = _write_control(root, "clean_lozenge_lattice", clean, width, height, svg_previews)

    broken = broken_noisy_strokes(clean, width, height, rng)
    broken_meta = _write_control(root, "broken_noisy_lozenge_lattice", broken, width, height, svg_previews)
    ...
    scratch_meta = _write_control(root, "random_scratches", scratches, width, height, svg_previews)
    ...
    curved_meta = _write_control(root, "curved_random_scratches", curved, width, height, svg_previews)
...
def compare_null_models(
    ...
    controls = _load_control_analyses(controls_dir)
    ...
    null_controls = [item for item in controls if "random" in item["label"]]
    lozenge_controls = [item for item in controls if "lozenge" in item["label"]]

    rows: list[dict[str, object]] = []
    for metric in DEFAULT_METRICS:
        null_values = [float(item["metrics"].get(metric, 0.0)) for item in null_controls]
        lozenge_values = [float(item["metrics"].get(metric, 0.0)) for item in lozenge_controls]
...
def _write_control(
    ...
    for stroke in strokes:
        ...
        if len(points) >= 2:
            cv2.polylines(image, [points], False, (230, 230, 230), 1, cv2.LINE_AA)
    cv2.imwrite(str(png_path), image)
    if svg_previews:
        _write_svg_preview(root / f"{name}.svg", strokes, width, height)
    return {
        "name": name,
        "image": str(png_path),
...
def _write_svg_preview(path: Path, strokes: list[Stroke], width: int, height: int) -> None:
    drawing = svgwrite.Drawing(str(path), size=(width, height), viewBox=f"0 0 {width} {height}")
    drawing.add(drawing.rect(insert=(0, 0), size=(width, height), fill="black"))
    for stroke in strokes:
        drawing.add(drawing.polyline(points=stroke.points, fill="none", stroke="white", stroke_width=1))
    drawing.save()
...
def _load_control_analyses(controls_dir: str | Path) -> list[dict[str, object]]:
    root = Path(controls_dir)
    controls: list[dict[str, object]] = []
    for graph_path in sorted(root.rglob("*.graphml")):
        controls.append({"label": graph_path.stem, "path": str(graph_path), "metrics": analyze_graphml(graph_path)})
    for metrics_path in sorted(root.rglob("*.json")):
        if metrics_path.name in {"comparison_summary.json", "lozenge_summary.json", "expected_metadata.json"}:
            continue
        if any(item["path"] == str(metrics_path) for item in controls):
            continue
        controls.append({"label": metrics_path.stem, "path": str(metrics_path), "metrics": _metrics_from_json(metrics_path)})
    return controls
